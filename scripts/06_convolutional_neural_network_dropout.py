...
Y4 = tf.nn.dropout(tf.nn.relu(tf.matmul(tf.reshape(Y3, shape=[-1, 7 * 7 * M]), W4) + B4), pkeep)
...
print "Max test accuracy achieved = {:.2f}%".format(max(test_accuracies) * 100) # 99.12%
...
plt.savefig("output_images/06_convolutional_neural_network_dropout.png")
